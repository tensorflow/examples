...
class TextSearcherClient(private var textSearcher: TextSearcher) {
    ...
    companion object {
        ...
        fun create(context: Context): TextSearcherClient {
            ...
            val searchOptions =
                SearcherOptions.builder().setMaxResults(MAX_RESULTS).setL2Normalize(IS_NORMALIZE)
                    .build()
            val options = TextSearcher.TextSearcherOptions.builder().setBaseOptions(baseOptions)
                .setSearcherOptions(searchOptions).build()
    ...
    fun search(query: String): List<Result> {
        ...
        modelResults.forEach {
            results.add(Result(it.distance, String(it.metadata.array())))
        }
        return results
    }
