...
public class QaClient implements AutoCloseable {
   ...
   */
  @WorkerThread
  public synchronized List<Answer> predict(String query, String content) {
    ...
    List<Answer> answers = getBestAnswers(startLogits[0], endLogits[0], feature);
    Log.v(TAG, "Finish.");
    return answers;
  }

  /** Find the Best N answers & logits from the logits array and input feature. */
  private synchronized List<Answer> getBestAnswers(
    ...
    List<Answer.Pos> origResults = new ArrayList<>();
  ...
  private synchronized int[] getBestIndex(float[] logits) {
    List<Answer.Pos> tmpList = new ArrayList<>();
