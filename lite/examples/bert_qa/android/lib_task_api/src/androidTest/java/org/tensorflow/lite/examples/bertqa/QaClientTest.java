...
public final class QaClientTest {
  ...
  public void testGoldSet() {
    ...
    List<Answer> predict0 = client.predict("What is Tesla's home country?", content);
    assertThat(getTexts(predict0)).contains("Serbian");
    List<Answer> predict1 = client.predict("What was Nikola Tesla's ethnicity?", content);
    assertThat(getTexts(predict1)).contains("Serbian");
    List<Answer> predict2 = client.predict("What does AC stand for?", content);
    assertThat(getTexts(predict2)).contains("alternating current");
    List<Answer> predict3 = client.predict("When was Tesla born?", content);
    assertThat(getTexts(predict3)).contains("10 July 1856");
  }

  private static List<String> getTexts(List<Answer> answers) {
